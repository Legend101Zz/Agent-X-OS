...
| G10 | **Creator Mandate** (assemble a Type from a description) | ✅ | **Hermes Phase 3 (this session):** ``build_creator_type()`` (``packages/mandate/src/agentx_mandate/library/creator.py``) returns a ``MandateType`` with the four §5 faculties (conversation + scheduling + memory-craft + escalation) + machine-checkable postconditions (candidate has faculties, has charter goal, names a scenario pack) + a substantive rubric (creator_quality with fit / charter_complete / scenario_pack criteria). Two new faculties: ``conversation`` (interview) and ``scheduling`` (cadence — emits the ``draft_candidate_type`` Call). New ``DraftCandidateTypeAdapter`` in the syscall lane stages the candidate ``MandateType`` as run output ONLY (no live registration — invariant #7); the adapter has zero catalog write paths and is structurally incapable of promoting. New ``draft_candidate_type`` policy in the gateway (L2, irreversible). 21 new tests (10 unit + 9 syscall + 1 playbook smoke + 1 library update to the Phase-1 faculty-set snapshot). | §5 |
